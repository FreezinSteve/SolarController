...
const uint32_t UNO_DATA_PERIOD = 60000;  // Data updates from UNO every 1 minutes
uint32_t unoTimer = 0;
byte solarOn = 0;
byte wetbackOn = 0;
byte boostState = 0;
...
//const char* proxyAddr = "192.168.1.68";
int proxyPort = 9000;
...
void setup(void) {
  ...
  //================================================
  // Default values
  strcpy(mNeonData[0], "0");
  strcpy(mNeonData[1], "0");
  strcpy(mNeonData[2], "0");
  strcpy(mNeonData[3], "0");
  strcpy(mNeonData[4], "0");
  strcpy(mNeonData[5], "0");
}
...
bool getSolarDataFromUNO(char* buff) {
  // Note, if module is connected then commands sent to the bluetooth module
  // may get to the UNO and affect operation
